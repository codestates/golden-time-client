...
export default class extends React.Component {
  ...
  makeTimer(closing_time) {
    ...
    if (diff < 0) {
      ...
      result = `남은시간 : ${diffDays < 10 ? `0${diffDays}` : diffDays}일 ${diffHours < 10 ? `0${diffHours}` : diffHours}시간 ${diffMin < 10 ? `0${diffMin}` : diffMin}분 ${diffSec < 10 ? `0${diffSec}` : diffSec}초`;
    }
  ...
  render() {
    const { userInfo, detail, imageNum, convertedData, comment, editingComment, editing } = this.state;
    return (
